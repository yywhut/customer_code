...
static void oz8806_change_data(struct battery_data *bat_data)
{
	int fg_current;

	bat_data->BAT_CAPACITY = oz8806_get_soc();
	bat_data->BAT_batt_temp = oz8806_get_battery_temp();
	bat_data->BAT_batt_vol = oz8806_get_battery_voltage();
	fg_current = oz8806_get_battry_current();

	if (bat_data->BAT_CAPACITY != -1) 
		set_rtc_spare_fg_value(bat_data->BAT_CAPACITY + 1); /*use battery_soc */

	bm_err("SOC %d,Ibat:%d,Vbat:%d Tbat:%d\n",
			bat_data->BAT_CAPACITY, fg_current, bat_data->BAT_batt_vol, bat_data->BAT_batt_temp);
}
static void oz8806_update(struct battery_data *bat_data)
{
	oz8806_battery_update_data();
	oz8806_change_data(bat_data);
}
#endif
...
int force_get_tbat(bool update)
{
	...
	#if 1 //jackymao
	if(bat_temperature_val>=50)
		bat_temperature_val=49;
	return bat_temperature_val;
	#else
	return bat_temperature_val;
	#endif
...
{
	bm_err("[BATTERY] wake_up_bat_bmu. \r\n");
