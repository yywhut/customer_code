...
static int oz8806_create_sys(struct device dev, const struct attribute_group * grp) //add by mao
{
	int err;

	batt_dbg("oz8806_create_sysfs");
	
	//if(NULL == dev){//add by mao
	//	pr_err("[BATT]: failed to register battery\n");
	//	return -EINVAL;
	//}

	//err = sysfs_create_group(&(dev->kobj), grp);
	err = sysfs_create_group(&(dev.kobj), grp);//add by mao
...
static void system_charge_discharge_status(struct oz8806_data *data)
{
	...
	batt_dbg("charger_type: %d", charger_type);
...
#endif
	{
		...
		else if ( (charger_type == STANDARD_HOST) ||
	...
	if (!capacity_init_ok && bmu_init_done) {
		...
		do {
			...
			if (battery_psy 
				...
				&& !battery_psy->desc->get_property (battery_psy, POWER_SUPPLY_PROP_CAPACITY, &val))//add by mao
			{
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = oz8806_create_sys(client->dev, &oz8806_attribute_group);//add by mao
	if(ret){
...
static int oz8806_remove(struct i2c_client *client)
{
	...
	if (battery_psy && !create_sys)
		...
		sysfs_remove_group(&(battery_psy->dev.kobj), &battery_psy_attribute_group);//add by mao
